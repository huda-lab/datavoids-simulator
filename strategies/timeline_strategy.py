...
class TimeLineStrategy(Strategy):
  """
  The strategy respects the timeline by pulling data from the nodes_info
  date_added field.
  """
  ...
  def initialize_strategy(self, agent: Agent):
    ...
    ordered_nodes = self.agent.graph.get_nodes_ordered_by_date_added(self.agent.group)
    pos_dict = {}
    for i, (n, d) in enumerate(ordered_nodes):
      pos_dict[n] = i
    self.plan_nodes.sort(key=lambda x: pos_dict[x], reverse=True)
